...
### 게시글 목록 화면
> 나중에 쓴 글이 최상단으로 올라오도록 sql문에 내림차순 정렬을 걸어두었습니다.
> 
...
<br /> 이후 비밀 비밀번호 체크 화면단에서는 `submit` 버튼 클릭시 `form` 태그를 통해 실제 삭제 요청을 전송합니다. <br/> 이때 위에서 전달받은 `id`를 포함하여 전송합니다.
...
<br /> `articleService` 에는 조금전 호출한 메서드가 구현되어 있습니다. <br/> 인자로 받을 값이 변경 될 가능성을 염두해 `ArticleDto.ArticlePasswordCheckRequest` 라는 공통 dto 사용하고 있습니다.
...
<br />게시글에서 수정하기 버튼 클릭 시 비밀번호 입력창으로 이동합니다. 이후 올바르게 입력 했다면 아래 화면으로 이동합니다.
